...
// Pathfinding: SERVER -> SECURITY -> ARCHIVE (optimal cost 7)
function routeToArchive(c: HeistController): void {
  expect(c.routeSelect('SECURITY')).toBe(true)
  expect(c.routeSelect(ARCHIVE_ID)).toBe(true)
}
...
describe('HeistController - pathfinding phase', () => {
  it('computes Dijkstra efficiency against the optimal route', () => {
    ...
    expect(p.playerCost).toBe(7)
    expect(p.optimalCost).toBe(7)
    expect(p.efficiency).toBe(100)
    expect(c.getView().phase).toBe('networkOptimization')
  })

  it('builds the route node-by-node and locks when reaching the archive', () => {
    const c = new HeistController('TEST')
    infiltrateToNetwork(c)
    c.interact()
    searchToTarget(c)
    c.interact()
    expect(c.getView().phase).toBe('pathfinding')
    expect(c.getView().routeCurrent).toBe(SERVER_ID)
    // Cannot jump to a non-adjacent node (ROUTER has no direct edge from SERVER)
    expect(c.routeSelect('ROUTER')).toBe(false)
    expect(c.getView().routePath).toEqual([SERVER_ID])
    // Build via BACKUP -> ROUTER -> ARCHIVE (valid route, cost 5+2+3=10)
    expect(c.routeSelect('BACKUP')).toBe(true)
    expect(c.routeSelect('ROUTER')).toBe(true)
    expect(c.getView().routeCost).toBe(7)
    // Undo the ROUTER hop and take a different branch
    expect(c.undoRoute()).toBe(true)
    expect(c.getView().routeCurrent).toBe('BACKUP')
    expect(c.resetRoute()).toBe(true)
    expect(c.getView().routePath).toEqual([SERVER_ID])
    // Optimal: SERVER -> SECURITY -> ARCHIVE
    expect(c.routeSelect('SECURITY')).toBe(true)
    expect(c.routeSelect(ARCHIVE_ID)).toBe(true)
    expect(c.getView().routeLocked).toBe(true)
    expect(c.getView().routeCurrent).toBe(ARCHIVE_ID)
    expect(c.interact()).toBe(true)
    expect(c.getView().phase).toBe('networkOptimization')
  })

  it('rejects route edits after locking', () => {
    const c = new HeistController('TEST')
    infiltrateToNetwork(c)
    c.interact()
    searchToTarget(c)
    ...
    routeToArchive(c)
    expect(c.getView().routeLocked).toBe(true)
    expect(c.routeSelect('CONTROL')).toBe(false)
    expect(c.undoRoute()).toBe(false)
    expect(c.resetRoute()).toBe(false)
  })
})
...
describe('scoring - transparent totals', () => {
  ...
  it('reflects penalties for a non-optimal run', () => {
    ...
    searchToTarget(c)
    c.interact()
    // Detour: SERVER -> CONTROL -> ARCHIVE (cost 10 vs optimal 7)
    c.routeSelect('CONTROL')
    c.routeSelect(ARCHIVE_ID)
    c.interact()
    solveMst(c)
    solveDp(c)
    c.finalizeDp()
...
describe('analysis - post-heist network', () => {
  it('builds the full facility graph from a completed snapshot', () => {
    ...
    for (const id of optimalRoute.slice(1, optimalRoute.indexOf(ARCHIVE_ID) + 1)) c.routeSelect(id)
    c.interact()
    solveMst(c)
    solveDp(c)
    c.finalizeDp()
  ...
  it('flags a single decision point when the player leaves the optimal route', () => {
    ...
    searchToTarget(c)
    c.interact()
    // Deliberately diverge through CONTROL instead of the optimal SECURITY branch.
    c.routeSelect('CONTROL')
    c.routeSelect(ARCHIVE_ID)
    c.interact()
    const analysis = buildAnalysis(c.toSnapshot())
    expect(analysis.pfDecisionPoint).not.toBeNull()
    expect(analysis.pfDecisionPoint!.nodeId).toBe(SERVER_ID)
    expect(analysis.pfDecisionPoint!.playerCost).toBeGreaterThan(0)
  })
})